const fs = require('fs');
const file = 'd:/Projects/React/Beam/Development/Beam2.o_South_Incidents/src/modules/safety-observations/pages/SOList.jsx';
...
function SOList() {
  ...
  const filtered = useMemo(() => {
    return OBSERVATIONS.filter(o => {
      ...
      if (searchTerm && !(o.id + o.subject + o.category).toLowerCase().includes(searchTerm.toLowerCase())) return false;
      return true;
    });
  }, [filterType, filterCategory, filterContractor, searchTerm]);
  ...
  return (
    <div className="mod-page">
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        <input type="text" className="mod-form-input" placeholder="Search ref, description..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ flex: 1, maxWidth: 300 }} />
